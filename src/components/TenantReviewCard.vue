...
<template>
  <div class="card mb-3">
    <div class="card-body">
      ...
      <div class="d-flex">
        ...
        <div>
          <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal"
            :data-bs-target="`#image_intake${props.data.tenant_rating_id}`">Add Image</button>

          <div class="modal fade" :id="`image_intake${props.data.tenant_rating_id}`" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-body">
                  <input class="form-control" type="url" :id="`img_url${props.data.tenant_rating_id}`"
                    placeholder="Paste link here..." />
                  <button @click.prevent="post_image(props.data.tenant_rating_id)" type="button" data-bs-dismiss="modal"
                    class="btn btn-outline-primary mt-2">Add Image</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <img class="img-thumbnail me-1 mt-1" v-for="image in images_ref" :key="image.image_id" :src="image.link">
